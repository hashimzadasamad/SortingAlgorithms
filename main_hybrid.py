...
import insertion, heap, radix_counting, quick, hybrid

x = 1
repeat = 100
with open('time_hybrid.txt', 'a') as file:
    file.write('size   insertion    heap   radix_counting   quick    hybrid\n')
    while x < 1001:
        for z in range(1, 10):
            size = x * z
            ...
            for _ in range(repeat):
                ...
                current_time_hybrid = timeit.timeit(lambda: hybrid.hybrid_sort(array[:]), number=1) * 1e9
                ...
                time_passed_hybrid += current_time_hybrid

            time_passed_insertion /= repeat
            time_passed_heap /= repeat
            time_passed_radix /= repeat
            time_passed_quick /= repeat
            time_passed_hybrid /= repeat

            file.write(f'{size}       {int(time_passed_insertion)}       {int(time_passed_heap)}        {int(time_passed_radix)}       {int(time_passed_quick)}       {int(time_passed_hybrid)}\n')
